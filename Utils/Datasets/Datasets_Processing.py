...
def generate_dataset(filepath):
    ...
    datasets = []  # 最终要保存的数据集合
    ###################################################################################
    # filename = FileOperation.get_datetime_str("datetime")  # 获得独特的时间戳以命名
    filepath = (
        FileOperation.get_BASE_DIR() + "/Datasets/Datasets" + "/" + "rome_trajectory"
    )
    # 获取存储路径
    ###################################################################################
    ...
    np.save(filepath, np.array(datasets))  # 保存为npy文件
    # 打印dataset形状
...
generate_dataset("rome_traces_coordinate.csv")
